...
function main() {
  ...
  const lookAtOpcodes = (code) =>
    code.forEach((line) =>
      seenOpcodes.add(line.match(/^(\w+).*/)[1].toUpperCase())
    );
  ...
  const tests = yaml.parse(content, (key, value) => {
    ...
    if (key === "state") {
      return Object.fromEntries(
        Object.entries(value).map(([address, account]) => [
          parseYamlBigInt(address),
          account,
        ])
      );
    }
  ...
  const ordered = Object.entries(tests)
    ...
    .map((t) => ({
      ...
      expect: {
        ...t.expect,
        success:
          typeof t.expect.success === "boolean" ? t.expect.success : true,
      },
    }));

  Object.keys(assembler.opcodes).forEach((name) => {
    if (
      !seenOpcodes.has(name) &&
      ["PUSH", "SWAP", "DUP"].every((prefix) => !name.startsWith(prefix))
    ) {
...
function parseYamlBigInt(value) {
  if (typeof value === "string" && value.match(/^(0x)?[0-9a-f_]+n$/i)) {
    return (
      "0x" + BigInt(value.replace(/n$/, "").replace(/_/g, "")).toString(16)
    );
  }
  return value;
}
