...
module.exports = function assembler(code) {
  const result = [];

  const instructions = [...code.matchAll(/([A-Za-z\d]+)/g)].map(
    (match) => match[1]
  );
  while (instructions.length > 0) {
    ...
    if (opcode >= OPCODES.PUSH1 && opcode <= OPCODES.PUSH32) {
      ...
      if (!value) {
        throw new Error(`Missing value for PUSH`);
      }
      ...
      if (n < 0 || n >= 2n ** BigInt(8 * length)) {
        throw new Error(`Value ${value} is not in range for PUSH${length}`);
      }

      for (let i = BigInt(length - 1); i >= 0; i--) {
        result.push(Number((n >> (8n * i)) & 0xffn));
      }
    }
  }
